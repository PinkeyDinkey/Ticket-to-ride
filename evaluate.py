...
from ticket_to_ride22 import predict_image
...
def evaluate_test_case(i):
    ...
    gt_centers = load_json(f'Data/{TRAIN_CASES[i]}_centers.json')
    gt_n_trains = load_json(f'Data/{TRAIN_CASES[i]}_n_trains.json')
    gt_scores = load_json(f'Data/{TRAIN_CASES[i]}_scores.json')

    img = cv2.imread(f'Data/{TRAIN_CASES[i]}.jpg')

    time_predict_start = time.time()
    centers, n_trains, scores = ticket_to_ride.predict_image(img)#predict_fn(img)
